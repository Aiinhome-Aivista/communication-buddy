...
import LoaderNew from "../../ui/LoaderNew";
import { useMinLoaderTime } from "../../../hooks/useMinLoaderTime";
>>>>>>> 687859ba82794287c33adef844c7b17ab49e31d9
...
const HrCandidateDashboard = () => {
  ...
  const barData = useMemo(() => {
    const list = isHR
      ? Array.isArray(data?.anuallyHiringProcess)
        ? data.anuallyHiringProcess
        : []
      : Array.isArray(data?.last_twelve_test_scores)
      ? data.last_twelve_test_scores
      : [];
  ...
  const technicalSkills = isHR
    ? Array.isArray(data?.top_technical_skills)
      ? data.top_technical_skills
      : []
    : Array.isArray(data?.most_discussed_technical_skills)
  ...
  const activeParticipant = isHR
    ? sessionReport?.activeParticipants ??
      sessionReport?.active_participant ??
      0
    : sessionReport?.test_attempted ??
      sessionReport?.assigned_test ??
      sessionReport?.activeParticipants ??
      sessionReport?.active_participant ??
      0;
  ...
  const sessionCreated = isHR
    ...
    : sessionReport?.assigned_test ??
      sessionReport?.test_assigned ??
      sessionReport?.session_created ??
      0;
  const userTraffic = isHR
    ? sessionReport?.userTraffic ?? sessionReport?.user_traffic ?? 0
    : sessionReport?.highest_score ??
      sessionReport?.best_score ??
      sessionReport?.average_score ??
      0;
  const averageScore =
  ...
  const topScores = isHR
    ? []
    : (data?.top_five_test_scores || []).map((s) => ({
        name: s.hr_name || "-",
        topic: s.topic || "-",
        score: s.score || "-",
<<<<<<< HEAD
...
<<<<<<< HEAD
  ...
  // Render loader at the top level to ensure it's centered on the whole screen
  if (showLoader) return <LoaderNew />;
  return ( // The main dashboard layout
    <div className="w-full bg-[#ECEFF2] p-3 overflow-visible">
      {/* Header */}
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold text-[#2C2E42]">Dashboard</h1>
        <div className="flex items-center gap-3">
          {isHR ? (
            <button
              className="flex items-center gap-2 bg-[#E5B800] hover:bg-yellow-500 text-xs text-[#272727] font-semibold px-4 py-2 rounded-xl shadow-none cursor-pointer"
                          ...
                          <Tooltip
                            ...
                            content={({ active, payload }) =>
                              active && payload && payload.length ? (
                                <div className="bg-[#FEEFC2] px-4 py-1 rounded-sm text-[#DFB916] font-normal text-sm shadow-md">
...
<div className="text-[#8F96A9]" style={{ fontSize: "20px", lineHeight: "100%" }}>
  {(() => {
    // Ensure it's a valid number and formatted with one decimal
